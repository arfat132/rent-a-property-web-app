import React, { useState, useContext } from 'react';
import { RiWallet3Line, RiArrowDownSLine, RiArrowUpSLine, } from 'react-icons/ri';
import { Menu } from '@headlessui/react';
import { PropertyContext } from './PropertyContext';

const PriceRange = () => {
    const { price, setPrice } = useContext(PropertyContext);
    const [isOpen, setIsOpen] = useState(false);

    const prices = [
        {
            value: 'Price range (any)',
        },
        {
            value: '100000 - 130000',
        },
        {
            value: '130000 - 160000',
        },
        {
            value: '160000 - 190000',
        },
        {
            value: '190000 - 220000',
        },
        {
            value: '20000 - 30000',
        },
        {
            value: '30000 - 40000',
        },
    ];

    return (
        <Menu as='div' className='dropdown relative'>
            <Menu.Button
                onClick={() => setIsOpen(!isOpen)}
                className='dropdown-btn w-full flex'
            >
                <RiWallet3Line className='dropdown-icon-primary text-xl text-violet-700 mr-1' />
                <div>
                    <div className='text-[15px] font-medium leading-tight'>{price}</div>
                    <div className='text-[13px] flex'>Choose price range
                        {isOpen ? (
                            <RiArrowUpSLine className='ml-2 mt-1' />
                        ) : (
                            <RiArrowDownSLine className='ml-2 mt-1' />
                        )}
                    </div>
                </div>
            </Menu.Button>
            <Menu.Items className='dropdown-menu list-none'>
                {prices.map((price, index) => {
                    return (
                        <Menu.Item
                            as='li'
                            onClick={() => setPrice(price.value)}
                            key={index}
                            className='cursor-pointer hover:text-violet-700 transition'
                        >
                            {price.value}
                        </Menu.Item>
                    );
                })}
            </Menu.Items>
        </Menu>
    );
};

export default PriceRange;